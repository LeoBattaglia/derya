...
class HTMLSourceCode {
    ...
    addTag(tag) {
        if (html_validation_1.isTag(tag.toLowerCase())) {
    ...
    closeTag(tag) {
        if (html_validation_1.isClosedTag(tag.toLowerCase())) {
    ...
    getHTML() {
        ...
        if (errors.getLength() > 0) {
            ...
            let lvl = 0;
            for (let element of this._sc) {
                if (element.tag && html_validation_1.isClosedTag(element.content) && element.closed) {
                    lvl--;
                }
                let tabs = "";
                for (let i = 0; i < lvl; i++) {
