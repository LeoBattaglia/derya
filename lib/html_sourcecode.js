...
class HTMLSourceCode {
    ...
    addContentUnsafe(content) {
        if (!sys.isNull(content)) {
            return this.addElement(new html_element_1.HTMLElement(this.getNewID(), content.trim(), false, undefined));
        }
        return undefined;
    }
    addDoctype() {
    ...
    static containsTags(sourceCode) {
    ...
    openScriptDefault(src) {
        let tag = this.openScript();
    ...
    parse(sourceCode) {
            ...
            if (index1 > 0) {
                ...
                if (HTMLSourceCode.isComment(tag)) {
                    ...
                    if (tag.toLowerCase().indexOf("doctype") > -1) {
                        ...
                        //console.log("TAG: " + tag);
                        let name = tag.substring(1, tag.length - 1);
        ...
        function createElement(tag, paras) {
            for (let p of paras) {
                //console.log("PARA: " + p);
                if (p.indexOf("=") < 0) {
                    //console.log("PARA1: " + p);
